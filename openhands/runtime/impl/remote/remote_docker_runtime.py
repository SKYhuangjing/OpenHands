...
class RemoteDockerRuntime(ActionExecutionClient):
    ...
    def __init__(
        ...
        if self.config.sandbox.runtime_container_image is None:
            raise ValueError(
                'runtime_container_image is required in the remote runtime.'
            )
    ...
    async def connect(self):
        try:
            ...
            self.log('error', f'Runtime failed to start: {e}')
            self.close()
            raise
    ...
    def _start_or_attach_to_runtime(self):
        ...
        if existing_runtime:
            ...
            assert (
                self.config.sandbox.runtime_container_image is not None
            ), "Remote Docker Runtime: runtime_container_image is required"
            self.log(
                'info',
    ...
    def _start_runtime(self):
        ...
        try:
            response = self._send_runtime_api_request(
                ...
                json=start_request,
                timeout=self.config.sandbox.remote_runtime_init_timeout,
            )
            self._parse_runtime_response(response)
            self.log(
                'debug',
    ...
    def _parse_runtime_response(self, response: requests.Response):
        start_response = get_data_from_response(response)
    ...
    def _wait_until_alive_impl(self):
        ...
        runtime_data = get_data_from_response(runtime_info_response)
    ...
    def close(self):
        ...
        if self.config.sandbox.keep_runtime_alive:
            if self.config.sandbox.pause_closed_runtimes:
                try:
                    if not self._runtime_closed and self.runtime_id is not None:
                        self._send_runtime_api_request(
                            'POST',
    ...
    def _send_runtime_api_request(self, method, url, **kwargs):
        try:
            ...
            if 'headers' not in kwargs:
                kwargs['headers'] = {}
            kwargs['headers']['session_id'] = self.sid
